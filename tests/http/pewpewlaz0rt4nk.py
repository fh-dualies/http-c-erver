...
class Beam(object):
    ...
    def __init__(self, request, response, description=None, host=None,
                 port=None, timeout=None, buffer_=4096, shutdown=False):
    ...
    def send(self):
        ...
        while total < length:
            count += 1
            chunk = self.request[total:total + self.buffer].encode(
                errors='ignore')
    ...
    def validate(self):
        ...
        for expected, received in zip(self.response, self._data.splitlines()):
            ...
            response.append(Response(
                self._line_valid(expected, received), expected, received))
        ...
        for i, beam in enumerate(self._beams, 1):
            ...
            for j, response in enumerate(beam.validate(), 1):
                ...
                # Invalid data
                    ...
                    pass_ = False
                    formatter = '[{0}] Failed\n{3}Expected: {1}\n{3}Received: {2}'
                    self._print_fail(formatter.format(
